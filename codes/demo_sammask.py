...
mask_generator = SamAutomaticMaskGenerator(model=sam,points_per_side = 96, pred_iou_thresh=0.93)
    ...
    with torch.no_grad():
        for image, image_path in test_loader:
            ...
            for i in range(output.size(0)):
                ...
                plt.clf()
                plt.imshow(mask_numpy, cmap='gray', vmin=0, vmax=1)
                plt.axis('off')
                file_name = os.path.basename(image_path[i])
                output_filename = f"./output_sam/{file_name}"
                ...
                plt.subplot(2, 2, 1)
                plt.imshow(Image.open(image_path[i]))
                ...
                plt.title("Sam Detection Result")
                plt.axis('off')

                plt.subplot(2, 2, 4)
                plt.imshow(filtered_output, cmap='gray', vmin=0, vmax=1)
                ...
                plt.show()
                file_name = os.path.basename(image_path[i])
                output_filename = f"output_dilateauto/{file_name}"
                plt.savefig(output_filename, bbox_inches='tight', pad_inches=0)
